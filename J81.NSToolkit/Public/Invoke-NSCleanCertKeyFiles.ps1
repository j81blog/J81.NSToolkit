function Invoke-NSCleanCertKeyFiles {
    <#
    .SYNOPSIS
        Cleanup old/unused certificates on a NetScaler.
    .DESCRIPTION
        With the Invoke-NSCleanCertKeyFiles function you can cleanup old (unused) CertKey files on your NetScaler. It will validate if a file is in use somewhere in the configuration, if not it will be marked for deletion. Then it will try to delete the marked files.
    ...
    .PARAMETER ManagementURL
        The URI/URL to connect to, E.g. "https://citrixNS.domain.local".
    ...
    .EXAMPLE
        PS C:\>Invoke-NSCleanCertKeyFiles -ManagementURL = "https://citrixNS.domain.local" -Credential $(Get-Credential -UserName "nsroot" -Message "NetScaler account")
        An example with only the required parameters, this will scan and delete unused certificates.
    .EXAMPLE
        PS C:\>Invoke-NSCleanCertKeyFiles -ManagementURL = "https://citrixNS.domain.local" -Credential $(Get-Credential -UserName "nsroot" -Message "NetScaler account") -Backup
        This example will first make a backup and then clean the NS.
    .NOTES
        File Name : Invoke-NSCleanCertKeyFiles.ps1
        Version   : v2204.1122
    ...
    Write-Verbose "Invoke-NSCleanCertKeyFiles: Starting"
    Write-Verbose "Trying to login into the NetScaler."
    $NSHASessions = Connect-NSHANodes -ManagementURL $ManagementURL -Credential $Credential -PassThru -DisplayConsoleText
    Write-Verbose $($NSHASessions | ConvertTo-Json -Depth 4)
    Write-Verbose "Finished connecting."
    $NSSessions = @()
    if ($NSHASessions.IsHA) {
        Write-Verbose "Session is HA"
        $NSSessions += $NSHASessions["SecondarySession"]
    }
    $NSSessions += $NSHASessions["PrimarySession"]
    Write-Verbose "Checking if there is a usable session..."
    if (-Not [String]::IsNullOrEmpty($($NSHASessions["PrimarySession"].Session))) {
        ...
        if ($Backup) {
            ...
            try {        
                ...
                $Response = Invoke-NSSaveNsconfig -all $true -NSSession $NSHASessions["PrimarySession"].Session -ErrorAction Stop
                $Response = Invoke-NSCreateSystembackup -NSSession $NSHASessions["PrimarySession"].Session -filename $BackupName -level full -comment "Backup created by PoSH function Invoke-NSCleanCertKeyFiles" -ErrorAction Stop
        ...
        try {
            ...
            Do {
                $Certs = Invoke-NSRetrieveCertificateRemoveInfo -NSSession $NSHASessions["PrimarySession"].Session
                ...
                if ($RemovableCerts.Count -gt 0) {
                    ...
                    Foreach ($RemovableCert in $RemovableCerts) {
                        ...
                        $RemovableCert | Where-Object { $_.certData.bound -eq $false } | ForEach-Object {
                            ...
                            $Response = Invoke-NSDeleteSslcertkey -NSSession $NSHASessions["PrimarySession"].Session -certkey $_.certData.certkey -ErrorAction SilentlyContinue | Write-NSText
                        ...
                        if ($_.keyData.certkey -ne $_.certData.certkey) {
                            $RemovableCert | Where-Object { $_.keyData.bound -eq $false } | ForEach-Object {
                                ...
                                Write-ConsoleText -Line "Removing"
                                $Response = Invoke-NSDeleteSslcertkey -NSSession $NSHASessions["PrimarySession"].Session -certkey $_.keyData.certkey -ErrorAction  SilentlyContinue | Write-NSText
                                Write-Verbose "Response: $Response"
                            }
                        }
                    }
            ...
            if ($RemovableCerts.Count -gt 0) {
                ...
                $RemovableCerts | ForEach-Object {
                    ...
                    foreach ($Session in $NSSessions) {
                        ...
                        $Response = Invoke-NSGetSystemfile -Filename  $_.fileName -Filelocation $_.filelocation -NSSession $Session.Session -ErrorAction SilentlyContinue | Expand-NSResult
                        if ($Response.filename -eq $_.fileName) {
                            $Response = Invoke-NSDeleteSystemfile -NSSession $Session.Session -filename $_.fileName -filelocation $_.filelocation -ErrorAction SilentlyContinue | Write-NSText
                        } else {
        ...
        Write-Verbose "Invoke-NSCleanCertKeyFiles: Ended"
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC