function Connect-NSHANodes {
    <#
    .SYNOPSIS
        Establish a session with (all) NetScalers in a HA configuration
    .DESCRIPTION
        Establish a session with (all) NetScalers in a HA configuration
    .PARAMETER ManagementURL
        The URL to connect to, E.g. "https://citrixNS.domain.local"
    .PARAMETER Credential
        The credential to authenticate to the NS with
    .PARAMETER Timeout
    ...
    .PARAMETER PassThru
        Return the NS session object
    .EXAMPLE
        PS C:\>Connect-NSHANode -ManagementURL https://citrixacd.domain.local -Credential (Get-Credential)
        Connect to the NS  nodes with specified management url and credential
    .NOTES
        File Name : Connect-NSHANodes
        Version   : v2204.1122
    ...
    begin {
        Write-Verbose "Connect-NSHANodes: Starting"
    ...
    process {
        Write-Verbose "Trying to login into the NetScaler."
        Write-ConsoleText -Title "NS Connection" -NoConsoleOutput:$NoConsoleOutput
        ...
        try {
            ...
            $NSHASession = Connect-NSNode -ManagementURL $ManagementURL -Credential $Credential -Timeout $Timeout -PassThru -ErrorAction Stop
            $IsConnected = $true
            Write-ConsoleText -ForeGroundColor Yellow -NoNewLine "*" -NoConsoleOutput:$NoConsoleOutput
            $HANode = Invoke-NSGetHanode -NSSession $NSHASession | Expand-NSResult
            $nsconfig = Invoke-NSGetNsconfig -NSSession $NSHASession | Expand-NSResult
            ...
            if ($NodeState -like "Stand-alone") {
                Write-ConsoleText -ForeGroundColor Yellow -NoNewLine "*" -NoConsoleOutput:$NoConsoleOutput
                if ($NSHASession.ContainsKey("IsStandalone")) { $NSHASession["IsStandalone"] = $true } else { $NSHASession.Add("IsStandalone", $true) }
                if ($NSHASession.ContainsKey("IsHA")) { $NSHASession["IsHA"] = $false } else { $NSHASession.Add("IsHA", $false) }
                try {
                    $PrimaryURL = [System.URI]"$($ManagementURL.Scheme):\\$($nsconfig.ipaddress)"
                    $PrimarySessionDetails = Connect-NSNode -ManagementURL $PrimaryURL -Credential $Credential -Timeout $Timeout -PassThru -ErrorAction Stop
                    $PriNode = $HANode | Where-Object { $_.ipaddress -eq $nsconfig.ipaddress }
                } catch {
                    $PrimarySessionDetails = $NSSession.psobject.copy()
                }
                $PrimarySession = @{
                    ID      = 0
                    State   = "Primary"
                ...
                $NSHASession = $PrimarySessionDetails.psobject.copy()
                if ($NSHASession.ContainsKey("PrimarySession")) { $NSHASession["PrimarySession"] = $NSHASession } else { $NSHASession.Add("PrimarySession", $PrimarySession) }
                Write-ConsoleText -ForeGroundColor Yellow -NoNewLine "*" -NoConsoleOutput:$NoConsoleOutput
                ...
                $NSHASession = $PrimarySessionDetails.psobject.copy()
                if ($NSHASession.ContainsKey("PrimarySession")) { $NSHASession["PrimarySession"] = $PrimarySession } else { $NSHASession.Add("PrimarySession", $PrimarySession) }
                Write-ConsoleText -ForeGroundColor Yellow -NoNewLine "*" -NoConsoleOutput:$NoConsoleOutput
                try {
                    ...
                    $SecondarySessionDetails = Connect-NSNode -ManagementURL $SecondaryURL -Credential $Credential -Timeout $Timeout -PassThru -ErrorAction Stop
                    Write-ConsoleText -ForeGroundColor Yellow -NoNewLine "*" -NoConsoleOutput:$NoConsoleOutput
                    $SecondarySession = @{
                        ID      = $SecNode.id
                        State   = $SecNode.state
                    ...
                    if ($NSHASession.ContainsKey("SecondarySession")) { $NSHASession["SecondarySession"] = $SecondarySession } else { $NSHASession.Add("SecondarySession", $SecondarySession) }
                    if ($NSHASession.ContainsKey("IsStandalone")) { $NSHASession["IsStandalone"] = $false } else { $NSHASession.Add("IsStandalone", $false) }
                    if ($NSHASession.ContainsKey("IsHA")) { $NSHASession["IsHA"] = $true } else { $NSHASession.Add("IsHA", $true) }
                } catch {
                    Write-Verbose "Error, $($_.Exception.Message)"
                    $SecondarySessionDetails = $null
                    if ($NSHASession.ContainsKey("SecondarySession")) { $NSHASession["SecondarySession"] = $null } else { $NSHASession.Add("SecondarySession", $null) }
                }
            }
        ...
        if ($IsConnected) {
            ...
            Write-ConsoleText -Title "NS Info" -NoConsoleOutput:$NoConsoleOutput
            ...
            if ($(ConvertFrom-NSVersion -NSSession $PrimarySessionDetails) -lt [System.Version]"11.0") {
                Throw "Only NS version 13 and up is supported, version 11.x & 12.x is best effort."
            }
        } else {
            $NSHASession = $null
        }
        $Script:NSLastSession = [PSObject]$PrimarySessionDetails.PSObject.Copy()
        Write-Output $NSHASession
    }
    end {
        Write-Verbose "Connect-NSHANodes: Ended"
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBNkRBpY4HGaPEd
# JTx8nXVV9Gr/dszW4qipayJyf03BfqCCHl4wggTzMIID26ADAgECAhAsJ03zZBC0
...
# hkiG9w0BCQQxIgQg/vaOEOfBr7VSyAqq+jtG1HBh+/S4SLxaqzZc41Z2sQ8wDQYJ
# KoZIhvcNAQEBBQAEggEAAig72Rc9Y+pZHsaJJPG+//udcTXbTvaGqHl/UGc9WzU8
# mIcTjg3SrjCkNDj9UO4oTfQoHXVq7JGwwhcbQWZb5F3qakv5epRFgSNk4PoSv83Q
# 4ON2CChB19E4nUvS+DYRMf0mO+q3xFrjQthosOBuFZV5a0VGT4FCU6Sf64HxK8Se
# Sy/dLp41ZbuIIs5K/fqQUkHuubqbU1/FWhxlJbibLjBlrxGel7yUBLPlS83ko5Mi
# V5rJUNUMG2WQTuWOvH0YCKCMiWHqwHrBXAowcxhcMPdECED7xN5e7UljlNKmwU/9
# RC1tIEFSkbPEoFNjqaen65EvVVqpA+OdamGnleUZgaGCA0swggNHBgkqhkiG9w0B
# CQYxggM4MIIDNAIBATCBkTB9MQswCQYDVQQGEwJHQjEbMBkGA1UECBMSR3JlYXRl
# ciBNYW5jaGVzdGVyMRAwDgYDVQQHEwdTYWxmb3JkMRgwFgYDVQQKEw9TZWN0aWdv
# IExpbWl0ZWQxJTAjBgNVBAMTHFNlY3RpZ28gUlNBIFRpbWUgU3RhbXBpbmcgQ0EC
# EDlMJeF8oG0nqGXiO9kdItQwDQYJYIZIAWUDBAICBQCgeTAYBgkqhkiG9w0BCQMx
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzExMTYyMDU4MDRaMD8GCSqG
# SIb3DQEJBDEyBDBRwVEh6uO4zkvrU7H5yPsOh5pN2Ca1votfPQwBj1PE0kTJcnqp
# j73OqGgw3hGgcGwwDQYJKoZIhvcNAQEBBQAEggIACANTu28oqqsWN+9DDYZi+dv1
# MzGr7y5aVgb2EpPrKI3k81Pi+AmjKL721yZSRXhzTqi9xj1YoWNfCNVLbkL5NIci
# w4jFyvvFdQulXnJXqwlNrAf3JXXqNA0T9gjIuzSUH29bdq5pi0B1V11+7F2WPEFb
# VhGUMHxFiwa4RB5EMk+Em2r6B6wZ5s3mqWssHV08uCca3wxzz1lUMZIkmzqMMRmD
# NzeKvHIR7BVdYXgkT63XGvTvezsRtDrzVxTXxuEwmdp7yCk/9YGzE5V5ex7ZLaMP
# cZEm7jgNMEUjcJlQ2g/0X3dlWdUZV0areTW4T16C314wuGNFHVqsKy3fFL4m1wXa
# 4qjS5WlIFdhusGr3v1LktIZjSKdCF6xoJQLOLb+ZEIbKSLg+2XsraWYtB02LNBoM
# 8qoMYqxZ+nzbgz1V1JHfn/8sm0wBGbQy0UwaOMidaKBX0TYrYZ2gZmQ80Jwe9iAP
# vr9V7zEkYVpoj9k8j5wPAuGxy8k9r2z9+xRLZ0HmqNrvldEpERmjpP1onfHqZ2FK
# PbOyalgj3pHwaTkxGmUvW31yvcM7co7YolPoVmgKAw3ReuIs2z5XaBIrVc2UqOQ9
# YI38gZBaQrzfsr7dQ0V+qMueQFaRAGf8WzxTJu5BbzTh79Zf9oLH+6pVpp0lDyoS
# EtoZ8i5e9MEFEb1J44o=
