function Connect-NSNode {
    <#
    .SYNOPSIS
        Establish a session with NetScaler.
    .DESCRIPTION
        Establish a session with NetScaler.
    ...
    .EXAMPLE
        PS C:\>Connect-NSNode -ManagementURL https://citrixacd.domain.local -Credential (Get-Credential)
    ...
    if ($Primary) {
        $NSSession = Connect-NSHANodes -ManagementURL $ManagementURL -Credential $Credential -PassThru -WarningAction SilentlyContinue
        $NSSession = $NSSession["PrimarySession"].Session
    } elseif ($HA) {
        $NSSession = Connect-NSHANodes -ManagementURL $ManagementURL -Credential $Credential -PassThru
    } else {
        $login = @{
            login = @{
...
"@ 
        ...
        try {
            ...
            $response = Invoke-RestMethod @params
            Write-Verbose -Message "Response:`n$(ConvertTo-Json -InputObject $response | Out-String)"
            $NSSession.version = ($response.nsversion.version.Split(","))[0]
            $NSSession.IsConnected = $true
        } catch {
